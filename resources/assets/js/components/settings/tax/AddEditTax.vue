        ...
        <div class="modal-layout-content">
            ...
            <form class="form-row" v-else>
                <div class="form-group col-md-12">
                    ...
                    <input v-validate="'required'" name="name" class="form-control" id="name" type="text" v-model="name">
                ...
                <div class="form-group margin-top col-md-6">
                    ...
                    <div class="heightError"><small class="text-danger" v-show="errors.has('percentage')">{{ errors.first('percentage') }}</small></div>
                </div>
                <div class="form-group margin-top offset-md-1 col-md-5">
                    <label>{{ trans('lang.is_default') }}</label><br>
                    <div class="custom-control custom-radio custom-control-inline">
                        <input type="radio" name="isDefault" class="custom-control-input" id="default" checked="checked" value="1" v-model="isDefault">
                        <label class="custom-control-label" for="default">{{ trans('lang.yes') }}</label>
                    </div>
                    <div class="custom-control custom-radio custom-control-inline">
                        <input type="radio" name="isDefault" :disabled="tempIsDefault==1" class="custom-control-input" id="not_default" value="0" v-model="isDefault">
...
<script>
    ...
    export default {
        ...
        created(){
            ...
            if(this.id)
            {
                this.getTaxData('/edittax/'+this.id);
            }
        },
        ...
        methods : {
            save()
            {
                ...
                if(this.id)
                {
                    this.postDataMethod('/edittax/'+this.id, this.inputFields);
                }
                else
                {
                    this.postDataMethod('/addtax',this.inputFields);
                }
                    }
                });
            },
