    ...
    <span>
        <div id="cart-section-1" class="modal-layout-header p-0">
            <div v-if="isSelectedBranch && customerNotAdded" class="sales-search p-2">
                <div class="row">
                    <div :class="{'col-10': addCustomer=='manage', 'col-12':(addCustomer!='manage' || salesOrReceivingType == 'internal'), 'pr-0':(order_type == 'sales' && salesOrReceivingType == 'customer' && addCustomer == 'manage')}">
        ...
        <div id="cart-section-3" class="position-absolute fixed-bottom product-card-font">
            ...
            <!--stated tax included or excluded-->
            <div class="row mx-0 px-3 py-2 font-weight-bold border-bottom"
                 v-if="currentBranch !== null && salesOrReturnType!=='returns' && order_type=='sales'">
                <div class="col-4 p-0 custom-line-height">
                    {{trans('lang.product_price_tax')}}
                </div>
                <div class="col-8 p-0 text-right">
                    <button class="btn btn-restaurant-order-type mr-2"
                            :class="{'selected-btn-restaurant-order-type': isTaxExcluded == true}"
                            @click.prevent="setTaxIncludedOrExcluded(true)">
                        {{trans('lang.excluded')}}
                    </button>
                    <button class="btn btn-restaurant-order-type"
                            :class="{'selected-btn-restaurant-order-type': isTaxExcluded == false}"
                            @click.prevent="setTaxIncludedOrExcluded(false)">
                        {{trans('lang.included')}}
                    </button>
                </div>
            </div>
            <!--end tax included or excluded-->
            <div class="row mx-0 px-3 py-2 font-weight-bold  border-bottom">
                <div class="col-6 p-0">
                    {{trans('lang.total')}}
                </div>
            ...
            <div class="p-3 border-bottom"
                 v-if="currentBranch !== null && currentBranch.branch_type === 'restaurant' && restaurantOrderType === 'dineIn' && salesOrReceivingType !== 'internal' && salesOrReturnType !== 'returns' && order_type === 'sales' && isPlaceOrderActive">
           ...
           <div v-if="(currentBranch !== null && currentBranch.branch_type !== 'restaurant' && salesOrReceivingType !== 'internal') || (currentBranch !== null && currentBranch.branch_type === 'restaurant' && salesOrReceivingType != 'internal'  && (order_type === 'receiving' || salesOrReturnType =='returns'))"
                class="p-3 border-bottom">
               <button class="btn pay-btn app-color"
                       data-toggle="modal"
                       data-target="#cart-payment-modal"
            ...
            <div v-if="order_type=='sales'" class="row mx-0">
                ...
                <a v-if="salesOrReceivingType== 'internal'"
                   href="#"
                   class="col-4 p-0 text-center border-right hold-items"
                   :class="{'disabled':internalHoldOrders.length == 0 ||cart.length != 0 || salesOrReturnType == 'returns' }"
    ...
    export default {
        ...
        methods: {
            ...
            allProductDiscount(value, index, unformatted) {
                this.$emit('allProductDiscountFromCart', value, index, unformatted);
            },
            ...
            setQuantityInCart(value, index) {
                ...
                if (this.cart[index].quantity > this.cart[index].availbleQuantity && this.order_type == 'sales') {
                    ...
                    let alertMessage = this.cart[index].productTitle + ' ' + variantTitle + ' ' + this.trans('lang.is_out_of_stock');
