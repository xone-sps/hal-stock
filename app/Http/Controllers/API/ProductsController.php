...
class ProductsController extends Controller
{
    ...
    public function getProduct(Request $request)
    {
        ...
        if (empty($request->reqType)) {
            return ['datarows' => $products['data'], 'allProducts' => $products['allProducts'],  'count' => $products['count']];
        } else {
    ...
    private function variantInsertion($request, $productId)
    {
        ...
        if (!empty($variantDetails)) {
            ...
            return ProductVariant::store(['product_id' => $productId->id, 'variant_title' => 'default_variant', 'attribute_values' => 'default_variant', 'purchase_price' => $receivingPrice, 'selling_price' => $sallingPrice, 'sku' => $sku, 're_order' => $reorder, 'bar_code' => $barcode, 'enabled' => $enabled]);
        }
    }
...
//            DB::rollback();
    ...
    public function adjustStockData(Request $request)
    {
        ...
        if ($branchId == null) {
            $branchId = Branch::getFirstObject()->id;
        }
        $data = $request->data;
        ...
        foreach ($data as $item) {
            ...
            $varientProduct = Product::allDetailsByVarientId($product['id']);
