...
class ReportController extends Controller
{
    ...
    public function inventoryReports(Request $request)
    {
        ...
        if (empty($requestType)) {
            return ['datarows' => $innventories, 'count' => $innventories];
        } else {
    ...
    {
        $branchId = Order::find($orderId)->branch_id;
        $isCashRegister = Branch::find($branchId)->is_cash_register;

        if ($isCashRegister == 0) {
            $cashRegisterId = null;
        } else {
            $cashRegisterId = CashRegister::find($branchId)->id;
        }
        $orderDetails = Order::orderDetails($orderId, $cashRegisterId);

        if ($cashRegisterId == null) {
            $invoiceTemplates = InvoiceTemplate::getInvoiceTemplateForNoCashReg($orderDetails->order_type);
        } else $invoiceTemplates = InvoiceTemplate::getInvoiceTemplateToPrint($cashRegisterId, $orderDetails->order_type);

        $discountAmount = OrderItems::getDiscountAmount($orderId);
        $exchange = Payments::getExchange($orderId);

        $itemDetails = OrderItems::getOrderDetails($orderId);
        $paymentDetails = Payments::paymentDetails($orderId);

        $invoiceLogo = Setting::getSettingValue('invoiceLogo')->setting_value;

        if ($orderDetails->table_id != null) {
            $orderTable = RestaurantTable::getOne($orderDetails->table_id);
            $orderDetails->table_name = $orderTable->name;
        }

        $orderDetails->phone_number = '';
        $orderDetails->address = '';

        if ($orderDetails->order_type == 'sales') {
            if ($orderDetails->sales_or_receiving_type == 'customer') {
                if ($orderDetails->customer_id != null) {
                    $orderCustomer = Customer::getOne($orderDetails->customer_id);
                    $orderDetails->customer_name = $orderCustomer->first_name . ' ' . $orderCustomer->last_name;
                    $orderDetails->phone_number = $orderCustomer->phone_number != null ? $orderCustomer->phone_number : '';
                    $orderDetails->address = $orderCustomer->address != null ? $orderCustomer->address : '';
                } else {
                    $orderDetails->customer_name = Lang::get('lang.walk_in_customer');
                }
            } else {
                $orderCustomer = Branch::getOne($orderDetails->transfer_branch_id);
                $orderDetails->customer_name = $orderCustomer->name;
            }
        } else {
            if ($orderDetails->sales_or_receiving_type == 'supplier') {
                if ($orderDetails->supplier_id != null) {
                    $orderCustomer = Supplier::getOne($orderDetails->supplier_id);
                    $orderDetails->customer_name = $orderCustomer->first_name . ' ' . $orderCustomer->last_name;
                } else {
                    $orderDetails->customer_name = Lang::get('lang.walk_in_supplier');
                }
            } else {
                $orderCustomer = Branch::getOne($orderDetails->transfer_branch_id);
                $orderDetails->customer_name = $orderCustomer->name;
        ...
        return [
            ...
            'customer_details' => $orderDetails->customer_name,
    ...
    public function customerSummaryReport(Request $request)
    {
        ...
        $customerReport = Order::customerReport($filtersData, $request->searchValue, $request->columnSortedBy, $limit, $request->rowOffset, $columnName, $requestType);
        $customerData = [];
        if (empty($requestType)) {
    ...
    public function supplierSummaryReport(Request $request)
    {
        ...
        $supplierReport = Order::supplierReport($filtersData, $request->searchValue, $request->columnSortedBy, $limit, $request->rowOffset, $columnName, $requestType);
        $supplierData = [];
