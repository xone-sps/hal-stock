...
{
    protected $fillable = ['date', 'order_type', 'sub_total', 'total_tax', 'all_discount', 'total', 'due_amount', 'type', 'profit', 'status', 'branch_id', 'transfer_branch_id', 'table_id', 'created_by', 'customer_id', 'supplier_id', 'invoice_id', 'created_at'];
    ...
    {
        return array_pluck(Order::distinct()->select('table_id')->where('status', 'hold')->get(), 'table_id');
    }
    ...
    public static function getTotalByType($data)
    {
        foreach ($data as $item) {
            $total_sales = Order::select(DB::raw('abs(sum(total)) as total_sales'))->where('customer_id', $item->customer_id)->where('total', '>', 0)->first();
            $item->total_sales = $total_sales->total_sales ? $total_sales->total_sales : 0;
            $total_return = Order::select(DB::raw('abs(sum(total)) as total_return'))->where('customer_id', $item->customer_id)->where('total', '<', 0)->first();
            $item->total_return = $total_return->total_return ? $total_return->total_return : 0;
            $total_payment = Order::select(DB::raw('abs(sum(total - due_amount)) as total_payment'))->where('customer_id', $item->customer_id)->where('total', '>', 0)->first();
    ...
    {
        $total_return_query = Order::select(DB::raw('sum(total) as total_return'))->where('order_type', 'sales')->where('total', '<', 0);

        $salesQuery = Order::select('orders.order_type',
            DB::raw('abs(sum(orders.due_amount)) as due'),
            DB::raw('abs(sum(orders.total_tax))  as tax'),
            DB::raw('sum(total) as total')
        )->where('order_type', 'sales')->where('total', '>', 0);
    ...
    public static function getCashRegisterInfo(){
